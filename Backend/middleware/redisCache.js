...
const isCacheableGetPath = (fullPath) => fullPath.startsWith("/api") || fullPath.startsWith("/menu/");
...
const shouldSkip = (req) => {
  ...
  return SKIP_CACHE_PATHS.some((prefix) => fullPath.startsWith(prefix));
};
...
const getNamespace = (req, fullPath) => {
  ...
  if (
    ...
    fullPath.startsWith("/menu/") ||
    fullPath === "/api/customer/search" ||
    fullPath === "/api/customer/restaurants/public-cuisines"
  ) {
  ...
  if (fullPath.startsWith("/api/customer")) {
    ...
    return username ? `customer:${username}` : "public:customer";
  }
