...
exports.increaseDishQuantity = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Dish quantity increased",
      data: {
        dish: dishName,
        quantity: cart[itemIndex]?.quantity || 0,
        cart: cart,
      },
    });
  } catch (error) {
...
exports.decreaseDishQuantity = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Dish quantity decreased",
      data: {
        dish: dishName,
        quantity: itemIndex > -1 ? cart[itemIndex]?.quantity || 0 : 0,
        removed: itemIndex > -1 ? cart[itemIndex] === undefined : true,
        cart: cart,
      },
    });
  } catch (error) {
...
// Order cart - clear cart array and return JSON
exports.orderCart = async (req, res, next) => {
  try {
    ...
    // Return cart and restaurant info for order confirmation
    return res.status(200).json({
      success: true,
